...
impl RigzFile for FileModule {
    fn read(&self, path: String, encoding: String) -> Result<String, VMError> {
        ...
        read_to_string(&path).map_err(|e| VMError::runtime(format!("Failed to read {path} - {e}")))
    }
    ...
    fn write(&self, path: String, contents: String, encoding: String) -> Result<(), VMError> {
        ...
        let mut file = File::open(&path)
            .map_err(|e| VMError::runtime(format!("Failed to open {path} - {e}")))?;
        file.write_all(contents.as_bytes())
            .map_err(|e| VMError::runtime(format!("Failed to write {path} - {e}")))?;
        Ok(())
    }
}
