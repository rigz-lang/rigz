...
impl Add for ValueRange {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        match (self, rhs) {
            (ValueRange::Int(a), ValueRange::Int(b)) => {
                Some(ValueRange::Int(Range {
            ...
            (ValueRange::Char(a), ValueRange::Char(b)) => {
                ...
                Some(ValueRange::Char(Range {
                    start,
                    end,
                }))
            }
            (ValueRange::Int(a), ValueRange::Char(b)) | (ValueRange::Char(b), ValueRange::Int(a)) => {
                let start = match char::from_u32(a.start as u32 + b.start as u32) {
                    None => return None,
                    Some(c) => c
                };
