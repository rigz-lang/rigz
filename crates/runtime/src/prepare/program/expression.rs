...
impl<'vm, T: RigzBuilder<'vm>> ProgramParser<'vm, T> {
    pub(crate) fn rigz_type(&self, expression: &Expression<'vm>) -> Result<RigzType, ValidationError> {
        let t = match expression {
            ...
            Expression::Identifier(a) => {
                match self.identifiers.get(a) {
                    None => {
                        match self.function_scopes.get(a) {
                            None => return Err(ValidationError::MissingExpression(format!("variable {a} does not exist"))),
                            Some(f) => {
                                match Self::function_call_return_type(a, f) {
                                    Ok(v) => v,
                                    Err(_) => return Err(ValidationError::MissingExpression(format!("variable {a} does not exist")))
                                }
                            }
                        }
                    },
            ...
            Expression::UnaryExp(o, e) => {
                match o {
                    ...
                    UnaryOperation::Print | UnaryOperation::EPrint | UnaryOperation::PrintLn | UnaryOperation::EPrintLn => {
                        RigzType::None
                    }
                }
            }
            ...
            Expression::FunctionCall(name, _) => {
                match self.function_scopes.get(name) {
                    None => return Err(ValidationError::InvalidFunction(format!("function {name} does not exist"))),
                    Some(f) => {
                        Self::function_call_return_type(name, f)?
                    }
                }
            }
            Expression::TypeFunctionCall(r, name, _) => {
                match self.function_scopes.get(name) {
                    None => return Err(ValidationError::InvalidFunction(format!("extension function {r}.{name} does not exist"))),
                    Some(f) => {
                        // todo ignore extension functions here
                        if f.len() > 1 {
                            // todo support union types
                            let matched: HashSet<_> = f.iter().filter_map(|(f, _)| match &f.self_type {
                                None => None,
                                Some((ft, _)) => {
                                    if &ft.rigz_type == r {
                            ...
                            match matched.len() {
                                0 => return Err(ValidationError::InvalidFunction(format!("extension function {r}.{name} does not exist"))),
            ...
            Expression::InstanceFunctionCall(_, calls, _) => {
                ...
                match self.function_scopes.get(name) {
                    None => return Err(ValidationError::InvalidFunction(format!("extension function {name} does not exist"))),
                    Some(f) => {
                        // todo ignore extension functions here
                        if f.len() > 1 {
                            // todo support union types
                            ...
                            match matched.len() {
                                0 => return Err(ValidationError::InvalidFunction(format!("extension {name} does not exist"))),
                                1 => matched.iter().next().cloned().unwrap(),
                                _ => RigzType::Any
                            }
                        } else {
    ...
    fn function_call_return_type(name: &&str, f: &FunctionCallSignatures) -> Result<RigzType, ValidationError> {
        let matched: HashSet<_> = f.iter().filter_map(|(f, _)| match &f.self_type {
        ...
        Ok(match matched.len() {
            0 => return Err(ValidationError::InvalidFunction(format!("function {name} does not exist"))),
            1 => matched.iter().next().cloned().unwrap(),
            _ => RigzType::Any
        })
    }
